...
import { uploadFile, convertPdfToDocx, getUsageInfo } from '../utils/api'

const SeoText = () => (
  <div className="prose max-w-none text-slate-300">
    <h2>What is PDF to DOCX conversion?</h2>
    <p>
      PDF to DOCX conversion transforms fixed-layout PDF documents into editable Word files (.docx). This
      allows you to update text, change formatting, and reuse content without retyping. Our tool extracts text,
      preserves layouts where possible, and produces a clean DOCX you can open in Microsoft Word, Google Docs,
      or other compatible editors.
    </p>
    <h2>How to convert PDF to DOCX?</h2>
    <p>
      Upload your PDF using the upload area above, then press the Convert button. The conversion runs in the
      cloud and you'll receive a DOCX download when the process completes. No software installation is
      required — the conversion is fast and handled on our secure servers.
    </p>
    <h2>Why use our tool?</h2>
    <p>
      Our PDF to DOCX converter is designed for simplicity and reliability. It works with complex PDFs,
      preserves most formatting, and respects your privacy — files are removed after processing. The tool is
      free to use for light conversions and integrates usage tracking so you can upgrade for higher limits.
    </p>
  </div>
)
...
const PdfToDocx = () => {
  ...
  useEffect(() => {
    document.title = 'Convert PDF to DOCX Online for Free - FilePro'
    const meta = document.querySelector('meta[name="description"]')
    const desc = 'Fast, secure, and easy PDF to DOCX conversion. Upload a PDF and download an editable DOCX.'
  ...
  const handleConvert = async () => {
    ...
    try {
      const data = await convertPdfToDocx(selectedFile)
      if (data?.file) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 py-8">
      <div className="mx-auto max-w-4xl px-4">
        <header className="mb-6">
          <h1 className="text-3xl font-extrabold text-white">Convert PDF to DOCX Online for Free</h1>
          ...
          <section className="pt-4">
            ...
            <div className="mt-6">
              ...
              <div className="mt-2 text-slate-300">
                ...
                <p className="mt-2"><strong>Q: What formats are supported?</strong> PDF input to DOCX output.</p>
              </div>
            </div>
          </section>
        </div>
